...
function send_sms($to_number, $message)
{
    ...
    $default_country = getenv('TWILIO_DEFAULT_COUNTRY') ?: '+880'; // Bangladesh country code

    // Check if Twilio credentials are configured
    if (!$sid || !$token || !$from) {
        error_log(date('Y-m-d H:i:s') . " - Twilio credentials not configured in environment variables\n", 3, __DIR__ . '/twilio_errors.log');
        return false;
    }
